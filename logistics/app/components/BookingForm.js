...
export default function BookingForm() {
    ...
    const updateResponses = (newData) => {
        ...
        setDriverResponses(prev => {
            // Use a Map to ensure uniqueness by driverId
            const map = new Map();
            
            // Add existing items to map
            prev.forEach(item => {
                if (item.driverId) map.set(item.driverId, item);
            });
    ...
    useEffect(() => {
        ...
        socketRef.current.on("driver_response", (data) => {
            console.log("Received driver response:", data);
            updateResponses(data);
        });

        socketRef.current.on("driver_counter_response", (data) => {
            console.log("Received driver counter:", data);
            updateResponses(data);
        });
    ...
    return (
        <>
            <section id="bookVehicle" className="min-h-screen w-full flex items-center justify-center px-4 py-8 sm:p-6 md:p-10 bg-gray-50 border-t-4 border-green-500">
                <div className="w-full max-w-2xl mx-auto p-6 sm:p-8 bg-white rounded-xl shadow-2xl">
                        ...
                        <div>
                            <div className="relative">
                                ...
                                <textarea
                                    ...
                                    {...register('details')}
                                    rows={4}
                                    className="w-full rounded-lg border border-gray-300 p-3 pl-10 text-sm shadow-sm"
                                ></textarea>
                            </div>
                        </div>

                        <button type="submit" className="w-full bg-green-600 mb-5 text-white py-3 px-5 rounded-lg font-semibold text-sm hover:bg-green-700 shadow-md">
                            Book Now & Get Quote
                        </button>
                    </form>
                    ...
                    {selectedPickup && selectedDropoff && (
                        <div className="col-span-full w-full h-64 mt-10 md:mt-0">
                            ...
                            {distance && <p className="text-center mt-2 text-gray-600">Estimated Distance: {distance}</p>}
                        </div>
                    )}

                    {driverResponse.map((resp, idx) => (
                        <div key={resp.driverId || idx} className="col-span-full flex flex-col gap-4 rounded-lg border bg-white p-4 shadow-sm mt-4">
                            {resp.status === 'accepted' && (
                                <>
                                    <p className="text-xl font-semibold text-green-600">Driver accepted your request! ✅</p>
                                    <ConfirmRide rideData={resp} onConfirm={() => handleConfirm(resp)} />
                                    <p className="text-sm text-gray-500">Please confirm within 10 seconds or this offer will expire.</p>
                                </>
                            )}
                            {resp.status === 'countered' && (
                                <>
                                    <p className="text-xl font-semibold text-gray-800">Driver sent a <span className="text-amber-500">counter-offer</span></p>
